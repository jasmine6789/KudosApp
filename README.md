...
| **Frontend tests**   | 77 passing, 98%+ coverage                                   |
...
- Vite + React with a conventional split: generic, reusable pieces live in `src/components/ui/`
  (`Button`, `Input`, `Textarea`, `Spinner`, `Toast`) and know nothing about shoutouts specifically. The
...
- **Typography:** Outfit throughout, self-hosted as a single variable-font file
  (`public/fonts/outfit-variable.woff2`) instead of pulled from Google Fonts at request time, so
  there is no external DNS/TLS round trip before the page can render its own type. The page title
  sits in a full-bleed black header band, bold and uppercase, always black regardless of the
  light/dark toggle.
...
| **HTTP security headers**  | Every response (both the API and the deployed frontend) sends `X-Content-Type-Options: nosniff`. The frontend additionally sends a `Content-Security-Policy` restricting scripts, styles, fonts, workers, and connections to known-safe origins (as of this pass, that policy allows nothing off the app's own origin at all, since the font is self-hosted, see [§4 Design System](#4-design-system)), `X-Frame-Options: DENY` to prevent clickjacking, `Referrer-Policy: strict-origin-when-cross-origin`, and a `Permissions-Policy` disabling camera/microphone/geolocation, none of which this app uses. |
...
- **Frontend (Vitest + React Testing Library):** 77 tests across the shared validation schema,
