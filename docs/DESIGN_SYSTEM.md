...
| **Success**                 | Form clears, a brief success toast appears with a pop-in animation and a layered confetti burst fires from the bottom two corners (`src/lib/confetti.ts`, skipped under reduced motion), and the new card animates into the grid (see §4).                                                |
