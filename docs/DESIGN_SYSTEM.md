...
| **Success**                 | Form clears, a brief success toast appears with a pop-in animation and a short confetti burst fires (`src/lib/confetti.ts`, skipped under reduced motion), and the new card animates into the grid (see §4).                                 |
...
| **Emoji picker**            | A row of emoji buttons, not a native `<select>`. Selected emoji gets `ring-2 ring-green-700`, a subtle scale (`scale-110`), and a gentle idle float; unselected are `opacity-70` until hovered/focused, and bounce briefly on hover.         |
...
- **Emoji picker interactions:** buttons scale and rotate slightly on hover/tap, and the checked emoji plays a gentle continuous float.
...
- **Confetti:** a short, on-brand confetti burst fires on a successful submit (`src/lib/confetti.ts`), wrapped in a try/catch so an environment without a working canvas context never breaks the actual submit flow over a visual flourish.
